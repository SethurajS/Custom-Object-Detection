...
#     ANCHORS = get_anchorss(cfg.YOLO.ANCHORS)

#     ANCHORS = get_anchorss(cfg.YOLO.ANCHORS_V3)
...
def draw_bbox(image, bboxes, classs, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        cls_ind = int(out_classes[0][i])
        if classes[cls_ind] == classs:
            coor = out_boxes[0][i]
...
def draw_bbox_all(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
